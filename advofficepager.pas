...
type
  ...
  TFloatingPagerWindow = class(TCustomForm)
  private
  ...
  public
    ...
    destructor Destroy; override;
  published
    property AdvPager: TAdvOfficePager read FAdvPager write FAdvPager;
  ...
  public
    ...
    function GetFloatingWindow: TFloatingPagerWindow;
  ...
  private
    ...
    FFloatingWindow: TFloatingPagerWindow;
    ...
    function GetPageFrom(PageName, PageCaption: string): TAdvOfficePage;
  protected
    procedure AlignControls(AControl: TControl; var ARect: TRect); override;
    procedure Loaded; override;
    procedure Notification(AComponent: TComponent; Operation: TOperation); override;
    ...
    procedure SetFloatingPage(Page: TAdvOfficePage; X: integer = 0; Y: integer = 0);
...
procedure DrawVistaGradient(Canvas: TCanvas; r: TRect; CFU, CTU, CFB, CTB, PC: TColor;
   ...
   Picture: TGDIPPicture; AntiAlias: TAntiAlias; RoundEdges: Boolean; RotateLeftRight: Boolean; Direction: TTabPosition; Progress: TTabProgress; IsOnGlass: Boolean); overload;
var
  graphics : TGPGraphics;
  path: TGPGraphicsPath;
...
procedure DrawVistaGradient(Canvas: TCanvas; r: TRect; CFU, CTU, CFB, CTB, PC: TColor;
   ...
   AntiAlias: TAntiAlias; RoundEdges: Boolean; RotateLeftRight: Boolean; Progress: TTabProgress; IsOnGlass: Boolean; Direction: TTabPosition = tpTop); overload;
begin
  DrawVistaGradient(Canvas, r, CFU, CTU, CFB, CTB, PC, GradientU,GradientB, Caption, AFont,
   nil, -1, True, Layout, False, False, Enabled, Focus, dpRight, nil, AntiAlias, RoundEdges, RotateLeftRight, Direction, Progress, IsOnGlass);
end;
    ...
    begin
      ...
      begin
        ...
        DrawVistaGradient(FBkgCache.Canvas, Rect(0, 0, FBkgCache.Width, FBkgCache.Height), Color, ColorTo, ColorMirror, ColorMirrorTo, BorderColor,
          Gradient, GradientMirror, '', Font, Layout, Enabled, False, FAdvOfficePager.AntiAlias, FAdvOfficePager.FCurrentOfficePagerStyler.RoundEdges, True, nil, IsOnGlass, FAdvOfficePager.TabPosition);
        Canvas.Draw(R.Left, R.Top, FBkgCache);
        FValidCache := true;
      end
      else
...
function TAdvOfficePage.GetFloatingWindow: TFloatingPagerWindow;
begin
  Result := nil;
  if not (csDesigning in ComponentState) and Assigned(Parent) then
  begin
    if Assigned(Parent.Parent) and (Parent.Parent is TFloatingPagerWindow) then
      Result := TFloatingPagerWindow(Parent.Parent);
  end;
end;
...
var
  i: Integer;
  floatingWndow: TFloatingPagerWindow;
begin
  Result := -1;
...
begin
  ...
  s := inttostr(AdvPageCount);
  sl.Values['PagesCount'] := s;
...
begin
  ...
  sl:= TStringList.Create;
  PageOrderList := TDbgList.Create;

  try
    Readln(F, s);  sl.CommaText := s;
    PC := StrToInt(sl.Values['PagesCount']);
    ...
    begin
      ...
      Readln(F, s);  sl.CommaText := s;
      if (sl.Values['TabVisible'] = 'True') then
        TabV := True
      else
        TabV := False;

      APage := GetPageFrom(N, C);
      if Assigned(APage) then
      begin
        APage.TabVisible := TabV;
      end
      else
      begin
        if (N <> '') or (C <> '') then
      ...
      if Assigned(APage) then
        PageOrderList.Add(APage);
    end;

    //--- set order
    if AdvPageCount >= PageOrderList.Count then
    begin
      for I := 0 to PageOrderList.Count - 1 do
      begin
        APage := TAdvOfficePage(PageOrderList[I]);
        if (I <> APage.GetPageIndex) then
          MoveAdvPage(APage.GetPageIndex, I);
      end;
    end;
...
function TAdvOfficePager.GetPageFrom(PageName,
  PageCaption: string): TAdvOfficePage;
var
  i: Integer;
begin
  Result := nil;
  if (PageName = '') and (PageCaption = '') then
    Exit;

  for I := 0 to FAdvPages.Count - 1 do
  begin
    if (PageName <> '') and (UpperCase(PageName) = UpperCase(AdvPages[I].Name)) or
       (PageName = '') and (UpperCase(PageCaption) = UpperCase(AdvPages[I].Caption)) then
    begin
      Result := AdvPages[I];
      Break;
    end;
      ...
      begin
        ...
        if UseOldDrawing then
          DrawVistaGradient(bmp.Canvas, Rect(0, 0, R.Right - R.Left, R.Bottom-R.Top),GradColor, GradColorTo, GradColorMirror, GradColorMirrorTo, PenColor,
            GradU, GradB, '', {Canvas.}Font, Layout, Enabled, False, FAntiAlias, True{FCurrentToolBarStyler.RoundEdges}, RotateLR, AdvPages[PageIndex].Progress, False, TabPos)
        else
        begin
          case TabPosition of
...
begin
  ...
  begin
    ...
    FFloatingWindow := TFloatingPagerWindow.CreateNew(Owner);
    FFloatingWindow.BorderIcons := FloatingBorderIcons;
...
constructor TFloatingPagerWindow.Create(AOwner: TComponent);
begin
  CreateNew(AOwner);
end;
...
constructor TFloatingPagerWindow.CreateNew(AOwner: TComponent; Dummy: Integer);
begin
  inherited;
  FOwner := AOwner;
end;
...
destructor TFloatingPagerWindow.Destroy;
begin
  inherited;
end;
...
function TFloatingPagerWindow.GetParentWnd: HWnd;
var
  Last, P: HWnd;
begin
...
procedure TFloatingPagerWindow.Loaded;
begin
  inherited;
end;
...
procedure TFloatingPagerWindow.CMMouseLeave(var Message: TMessage);
begin
  inherited;
end;
...
procedure TFloatingPagerWindow.Paint;
begin
  inherited;
end;
...
procedure TFloatingPagerWindow.WMActivate(var Message: TWMActivate);
begin
  inherited;
  ...
  else if Assigned(FAdvPager) then
  begin
    //FAdvPager.SetFocus;
...
procedure TFloatingPagerWindow.WMClose(var Message: TMessage);
begin
  inherited;
end;
...
procedure TFloatingPagerWindow.WMNCHitTest(var Message: TWMNCHitTest);
begin
  inherited;
end;
