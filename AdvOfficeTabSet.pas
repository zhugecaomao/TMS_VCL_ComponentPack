...
type
  ...
  private
    ...
    function GetTabFrom(TabCaption: string): TOfficeTabCollectionItem;
  protected
...
begin
  ...
  s := inttostr(FAdvOfficeTabs.Count);
  sl.Values['TabsCount'] := s;
  Writeln(F, sl.CommaText);

  for I := 0 to FAdvOfficeTabs.Count - 1 do
  begin
    sl.Clear;
    s := AdvOfficeTabs[I].Caption;
    sl.Values['Caption'] := s;
    Writeln(F, sl.CommaText);
...
begin
  ...
  try
    ...
    TC := StrToInt(sl.Values['TabsCount']);
    ...
    begin
      ...
      Readln(F, s);  sl.CommaText := s;
      if (sl.Values['Visible'] = 'True') then
        TabV := True
      else
        TabV := False;
      ...
      if Assigned(ATab) then
        TabOrderList.Add(ATab);
    end;

    //--- set order
    if AdvOfficeTabs.Count >= TabOrderList.Count then
    begin
      for I := 0 to TabOrderList.Count - 1 do
      begin
        ATab := TOfficeTabCollectionItem(TabOrderList[I]);
        if (I <> ATab.Index) then
          AdvOfficeTabs.Move(ATab.Index, I);
      end;
    end;
...
function TAdvCustomOfficeTabSet.GetTabFrom(TabCaption: string): TOfficeTabCollectionItem;
var
  i: Integer;
begin
  Result := nil;
  ...
  for I := 0 to FAdvOfficeTabs.Count - 1 do
  begin
    if (UpperCase(TabCaption) = UpperCase(FAdvOfficeTabs[I].Caption)) then
    begin
      Result := FAdvOfficeTabs[I];
      Break;
    end;
  end;
end;
